...
class LocationPagingSourceOnline @AssistedInject constructor(
    ...
    override suspend fun getCount(): Int {
        return locationDao.getPageKeysCount(locationFilter.mapToEntity())
    }

    override suspend fun getData(limit: Int, offset: Int): List<Location> {
        val locationIds = locationDao.getLocationIds(
            locationFilter.mapToEntity(),
            limit,
            offset
        )
        return locationDao.getLocationsByIds(locationIds).map { it.mapToLocation() }
    }
